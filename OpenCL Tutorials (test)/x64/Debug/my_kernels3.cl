...
__kernel void max_finder(__global const int* A, __global int* B) {
	int id = get_global_id(0);
	int N = get_global_size(0);
	float  Min = 0;
 ...
 barrier(CLK_GLOBAL_MEM_FENCE);
	...
	for (int i = 1; i < N; i *= 2) { //i is a stride
		if (!(id % (i * 2)) && ((id + i) < N) && (B[id] != 2147483647)) 
		if (B[id] < B[id + i])
			B[id] = B[id + i];
...
//find average value in dataset 
__kernel void avg_finder(__global const int* A, __global int* B) {
	int id = get_global_id(0);
	int N = get_global_size(0);
...
__kernel void hist_simple(__global const int* A, __global int* H, int bin_width, int max_value, int min_value) { 
	int id = get_global_id(0);

	//assumes that H has been initialised to 0
	int bin_index = A[id]-1;
	int width = bin_index / bin_width;

	barrier(CLK_GLOBAL_MEM_FENCE);

	if (bin_index > max_value) 
	{
		//atomic_inc(&H[max_value]);
		return;
	}
	else
	{
		atomic_inc(&H[width]);//serial operation, not very efficient!
