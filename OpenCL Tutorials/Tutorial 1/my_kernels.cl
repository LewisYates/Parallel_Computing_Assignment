...
__kernel void hist_simple(__global int* temperature, __global int* output, int bin_width) { 
	int id = get_global_id(0);
	...
	int width = bin_index / bin_width;
		...
		if (bin_index > 100000) 
	{
		//atomic_inc(&temperature[max_value]);
		return;
	}
	else
	{
		atomic_inc(&temperature[width]);//serial operation, not very efficient;
	}	
}
